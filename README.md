...
- **Consume:** link the immutable CDN tag `cdn.jsdelivr.net/gh/vititth-eng/ccd-design-system@<tag>/v2/*.css` — read the current tag off a live consumer's `app/layout.tsx`; bump all consumers in lockstep, never `@main`. Frame is CSS-only `v2/shell.css` — apps author their own markup + copy the ~15 glue lines from `v2/gallery/shell-gallery.html`.
