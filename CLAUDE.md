...
**Before cutting a tag** read `~/Desktop/Superpowers/wiki/app-design-method.md` · *Version numbering*. It is NOT plain semver: the major digit is the DS **generation** (`v2.x` = second generation), so breaking changes ride the **minor** slot with `BREAKING:` opening the message. `v3` is claimed as of 2026-08-11 — React + Tailwind + Base UI, piloted in `ccd-brb-multi-rater`, the whole app (owner call 2026-08-14; debrief held this slot until then — Linear CCD-281). It is a different delivery model, not a renamed v2: components are copied into each app by the shadcn CLI instead of linked from this repo's CDN. v2 keeps shipping unchanged until that pilot says otherwise. Always `git tag -a` — the annotated tag message is the changelog, there is no CHANGELOG.md.
