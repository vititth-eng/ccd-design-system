...
Graduation = ONE atomic commit: edit v2 source → switch every consumer to `var()`/the class, delete all literals (no alias) → update the catalog (`v2/*-gallery.html`; new component also gets a `v2/showroom.html` tile). Undemoed = will be re-invented. (The old fourth step re-pinned mocks under a workspace `plan/` directory; that store no longer exists — plans live in Linear.)
